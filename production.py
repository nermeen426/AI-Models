...
# Function to execute Cypher query and handle errors
def execute_query_with_retry(cypher_query):
    try:
        with driver.session() as session:
            ...
            if not records:
                return "No data found for the given query. Ensure that `deleted_at IS NULL` and `organization_idd` are correct."
...
if __name__ == "__main__":
    ...
    # Process the query with the given organization ID
    cypher_query, result = query_database_with_org_id(natural_language_query, organization_id)

    print("\nGenerated Cypher Query:")
    print(cypher_query)  # Print the generated Cypher query
